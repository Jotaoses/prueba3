...
public class ControladorUsuario extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        switch(accion){
            case "1": iniciarSesion(request,response);
                break;
            default: response.sendRedirect("login.jsp?msj=Debe Iniciar Sesion");
        }
        }else{
            response.sendRedirect("login.jsp?msj=Debe Iniciar Sesion");
        }
    }
    ...
    private void iniciarSesion(HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        if(temporal!=null){
            ...
            response.sendRedirect("floreria.jsp");
            }else{
                response.sendRedirect("login.jsp?msj=Password Incorrecta");
            }
        }else{
            response.sendRedirect("login.jsp?msj=Admin no registrado");
        }
        }catch(Exception e){
